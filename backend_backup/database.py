...
from models import DeviceData, DeviceReading, ElectricityRate, RateType, User, UserCreate, UserRole
...
class DatabaseManager:
        ...
        for row in rows:
            ...
            if power_w:
                device_data[device_ip]['peak_demand_kw'] = max(
                    device_data[device_ip]['peak_demand_kw'],
                    power_w / 1000
                )
    ...
    async def update_device_ip(self, old_ip: str, new_ip: str) -> bool:
        ...
        try:
            ...
            cursor = await self.sqlite_conn.execute(
                "SELECT COUNT(*) FROM device_info WHERE device_ip = ?", 
                (new_ip,)
            )
    ...
    async def remove_device(self, device_ip: str) -> bool:
        ...
        try:
            ...
            await self.sqlite_conn.execute(
                "DELETE FROM device_readings WHERE device_ip = ?", 
                (device_ip,)
            )
            
            # Delete device costs
            await self.sqlite_conn.execute(
                "DELETE FROM device_costs WHERE device_ip = ?", 
                (device_ip,)
            )
            
            # Delete device info
            await self.sqlite_conn.execute(
                "DELETE FROM device_info WHERE device_ip = ?", 
                (device_ip,)
            )
