...
class ExportRetentionScheduler:
    ...
    def __init__(
        ...
        self.enable_scheduler = (
            os.getenv("EXPORT_RETENTION_ENABLED", "true").lower() == "true"
        )

        logger.info(
            f"Export retention scheduler initialized - "
            ...
            try:
                ...
                if await self._should_run_daily_cleanup(now):
                    ...
                    try:
                        ...
                        logger.info(
                            f"Daily maintenance completed: {len(results.get('tasks_completed', []))} tasks"
                        )
                ...
                if await self._should_check_storage(now):
                    try:
                        storage_info = (
                            await self.retention_service.check_storage_space()
                        )
                        ...
                        if storage_info.get("needs_emergency_cleanup", False):
                            logger.warning(
                                "Emergency cleanup triggered by low disk space"
                            )
                            deleted, freed_mb = (
                                await self.retention_service.emergency_cleanup()
                            )
                ...
                if now.minute == 0:  # Run at the top of every hour
                    try:
                        ...
                        marked = (
                            await self.retention_service.mark_exports_expiring_soon()
                        )
                        if marked > 0:
                        ...
                        deleted, updated = (
                            await self.retention_service.cleanup_expired_exports(
                                batch_size=10
                            )
                        )
                        if deleted > 0 or updated > 0:
                            logger.info(
                                f"Hourly cleanup: {deleted} files deleted, {updated} records updated"
                            )
    ...
    async def _should_check_storage(self, now: datetime) -> bool:
        ...
        if 8 <= now.hour <= 18:
            ...
            if not self.last_storage_check or (
                now - self.last_storage_check
            ) >= timedelta(hours=1):
                return True
        else:
    ...
    async def force_storage_cleanup(self) -> Dict:
        ...
        try:
            ...
            logger.info(
                f"Forced storage cleanup completed: {deleted} files deleted, {freed_mb}MB freed"
            )
            return results
    ...
    def get_scheduler_status(self) -> Dict:
        ...
        return {
            ...
            "check_interval_minutes": self.check_interval.total_seconds() / 60,
            "last_daily_cleanup": (
                self.last_daily_cleanup.isoformat() if self.last_daily_cleanup else None
            ),
            "last_storage_check": (
                self.last_storage_check.isoformat() if self.last_storage_check else None
            ),
    ...
    def _get_next_daily_cleanup(self) -> str:
        ...
        next_cleanup = now.replace(
            hour=self.cleanup_hour, minute=0, second=0, microsecond=0
        )
...
def initialize_scheduler(
    ...
    **kwargs,
    ...
    _scheduler_instance = ExportRetentionScheduler(
        retention_service=retention_service, audit_logger=audit_logger, **kwargs
    )
