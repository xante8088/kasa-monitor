...
class ExportRetentionConfig:
    ...
    async def update_retention_policies(
        self, policies: Dict[str, int], user_id: Optional[int] = None
    ) -> bool:
    ...
    async def get_cleanup_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "check_interval_minutes": await self.get_config(
                "export.cleanup.check_interval_minutes", 60
            ),
        }
    ...
    async def get_storage_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "emergency_threshold_gb": await self.get_config(
                "export.storage.emergency_threshold_gb", 1
            ),
            "warning_threshold_gb": await self.get_config(
                "export.storage.warning_threshold_gb", 5
            ),
            "large_file_threshold_mb": await self.get_config(
                "export.storage.large_file_threshold_mb", 100
            ),
        }
    ...
    async def get_user_config(self) -> Dict[str, Any]:
        ...
        return {
            "admin_bonus_days": await self.get_config(
                "export.user.admin_bonus_days", 14
            ),
            "frequent_access_bonus_days": await self.get_config(
                "export.user.frequent_access_bonus_days", 7
            ),
            "frequent_access_threshold": await self.get_config(
                "export.user.frequent_access_threshold", 5
            ),
        }
    ...
    async def import_config(
        self, config_data: Dict, user_id: Optional[int] = None
    ) -> Dict:
...
def load_config_from_env() -> Dict[str, Any]:
    ...
    if "EXPORT_MAX_STORAGE_GB" in os.environ:
        env_config["export.storage.max_size_gb"] = os.getenv(
            "EXPORT_MAX_STORAGE_GB", "50"
        )

    if "EXPORT_EMERGENCY_CLEANUP_THRESHOLD_GB" in os.environ:
        env_config["export.storage.emergency_threshold_gb"] = os.getenv(
...
async def apply_env_config(
    config_manager: ExportRetentionConfig, user_id: Optional[int] = None
) -> int:
