...
class PluginSigner:
    ...
    def sign_plugin(
        ...
        if not output_path:
            ...
            output_path = str(base_path.parent / f"{base_path.stem}_signed{base_path.suffix}")
        ...
        with zipfile.ZipFile(output_path, 'w', zipfile.ZIP_DEFLATED) as signed_zip:
            ...
            signed_zip.writestr('SIGNATURE.sig', base64.b64encode(signature).decode('utf-8'))
        ...
        try:
            with zipfile.ZipFile(plugin_path, 'r') as plugin_zip:
                ...
                for key_name, public_key in self.trusted_keys.items():
                    try:
                        if signature_data.get("algorithm") == SignatureAlgorithm.RSA_PSS_SHA256.value:
                            public_key.verify(
                                signature,
                                signature_bytes,
                                padding.PSS(
                        ...
                        result["errors"].append(f"Verification error with key {key_name}: {str(e)}")
...
class PluginSecurityManager:
    ...
    def check_plugin_security(self, plugin_path: str) -> Dict[str, Any]:
        ...
        if trust_hierarchy[verification["trust_level"]] < trust_hierarchy[min_trust]:
            result["errors"].append(f"Plugin trust level {verification['trust_level'].value} below minimum {min_trust.value}")
            return result
        
        # Check if unsigned plugins are allowed
        if not self.policies["allow_unsigned"] and verification["trust_level"] == TrustLevel.UNSIGNED:
        ...
        if verification["trust_level"] == TrustLevel.UNSIGNED:
            ...
            result["warnings"].append("Plugin signed by community member - verify source")
