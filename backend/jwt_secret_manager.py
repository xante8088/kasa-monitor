...
class JWTSecretManager:
    ...
    def _ensure_secure_permissions(self) -> None:
        ...
        if self.secret_file.exists():
            ...
            if file_stat.st_mode & 0o077:  # Check if group/other have any permissions
                logger.warning(
                    f"Secret file {self.secret_file} has insecure permissions"
                )
    ...
    def get_current_secret(self) -> str:
        ...
        if "current" in secrets_data:
            ...
            if (
                datetime.fromisoformat(current_data["created"]) + timedelta(days=90)
                > datetime.now()
            ):
    ...
    def get_secret_info(self) -> Dict[str, Any]:
        ...
        info = {
            ...
            "has_previous": "previous" in secrets_data
            and len(secrets_data["previous"]) > 0,
            "file_exists": self.secret_file.exists(),
            "file_permissions": (
                oct(os.stat(self.secret_file).st_mode)[-3:]
                if self.secret_file.exists()
                else None
            ),
        }
            ...
            info.update(
                {
                    ...
                    "current_age_days": (
                        datetime.now() - datetime.fromisoformat(current["created"])
                    ).days,
                }
            )
