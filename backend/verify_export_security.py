...
def verify_data_export_api_security():
    ...
    try:
        spec = importlib.util.spec_from_file_location(
            "data_export_api", Path(__file__).parent / "data_export_api.py"
        )
    ...
    if hasattr(data_export_api, "DataExportAPIRouter"):
        ...
        if hasattr(router_class, "_setup_routes"):
            ...
            rate_limit_checks = ["_check_export_rate_limit", "rate_limit_exceeded"]
    ...
    if hasattr(data_export_api, "DataExportAPIRouter"):
        ...
        expected_methods = ["_check_export_rate_limit", "_estimate_record_count"]
    ...
    try:
        spec = importlib.util.spec_from_file_location(
            "data_export_service", Path(__file__).parent / "data_export_service.py"
        )
    ...
    if critical_passed == len(critical_tests):
        ...
        print(
            f"❌ CRITICAL SECURITY ISSUES REMAIN: {len(critical_tests) - critical_passed} failures"
        )
