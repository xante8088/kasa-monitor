...
class DataAggregator:
    ...
    async def aggregate_hourly_data(self, force_date: Optional[datetime] = None):
        ...
        for device in devices:
            ...
            result = await self.db_manager.execute_query(
                query,
                [device_ip, target_hour.isoformat(), next_hour.isoformat()]
            )
    ...
    async def calculate_statistics(
        ...
        results = await self.db_manager.execute_query(
            query,
            [device_ip, start_date.isoformat(), end_date.isoformat()]
        )
        ...
        stats = {
            ...
            'power': {
                ...
                'percentiles': {
                    ...
                    '95': statistics.quantiles(power_values, n=20)[18] if len(power_values) > 19 else max(power_values),
                }
            },
            'energy': {
    ...
    async def _insert_hourly_summary(self, **kwargs):
        ...
        await self.db_manager.execute_query("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, [
            kwargs['device_ip'], kwargs['hour'].isoformat(), kwargs['count'],
            kwargs['avg_power'], kwargs['max_power'], kwargs['min_power'],
            kwargs['total_energy'], kwargs['avg_voltage'], kwargs['avg_current']
        ])
    ...
    async def _insert_daily_summary(self, **kwargs):
        ...
        await self.db_manager.execute_query("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, [
            kwargs['device_ip'], kwargs['date'].isoformat(), kwargs['total_kwh'],
            kwargs['peak_power_w'], kwargs['average_power_w'], kwargs['min_power_w'],
            kwargs['on_time_hours'], kwargs['cost']
        ])
    ...
    async def _insert_monthly_summary(self, **kwargs):
        ...
        await self.db_manager.execute_query("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, [
            kwargs['device_ip'], kwargs['year'], kwargs['month'], kwargs['days'],
            kwargs['total_energy'], kwargs['avg_power'], kwargs['max_power'],
            kwargs['total_cost'], kwargs['total_on_time']
        ])
