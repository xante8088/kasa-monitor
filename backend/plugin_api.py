...
from plugin_system import PluginLoader, PluginManifest, PluginState, PluginType, PluginPriority
...
class PluginAPIRouter:
    ...
    def __init__(self, app: FastAPI, plugin_loader: PluginLoader, 
                 hook_manager: HookManager, db_path: str = "kasa_monitor.db"):
    ...
    def _setup_routes(self):
        ...
        async def get_plugin_logs(plugin_id: str, limit: int = 100,
        ...
        async def upload_and_install_plugin(file: UploadFile = File(...),
                                           enable_after_install: bool = False,
    ...
    async def _enable_plugin_task(self, plugin_id: str):
        ...
        try:
            ...
            if success:
                # Emit event
                await self.hook_manager.emit("plugin.enabled", {
                    'plugin_id': plugin_id,
                    'timestamp': datetime.now().isoformat()
                })
        except Exception as e:
            # Log error
            await self.hook_manager.emit("plugin.error", {
                'plugin_id': plugin_id,
                'error': str(e),
                'timestamp': datetime.now().isoformat()
            })
    
    async def _install_plugin_task(self, source: str, install_dependencies: bool,
        ...
        try:
            ...
            if plugin_id:
                ...
                # Emit event
                await self.hook_manager.emit("plugin.installed", {
                    'plugin_id': plugin_id,
                    'timestamp': datetime.now().isoformat()
                })
            ...
            # Emit error event
            await self.hook_manager.emit("plugin.install_error", {
                'source': source,
                'error': str(e),
                'timestamp': datetime.now().isoformat()
            })
        ...
        try:
            ...
            if update_url:
                ...
                if new_plugin_id:
                    ...
                    # Emit event
                    await self.hook_manager.emit("plugin.updated", {
                        'plugin_id': plugin_id,
            ...
            # Emit error event
            await self.hook_manager.emit("plugin.update_error", {
                'plugin_id': plugin_id,
                'error': str(e),
                'timestamp': datetime.now().isoformat()
            })
