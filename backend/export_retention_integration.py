...
class ExportRetentionSystem:
    ...
    def __init__(
        ...
        self.config_manager = ExportRetentionConfig(
            db_path=db_path, audit_logger=self.audit_logger
        )
    ...
    async def initialize(self, apply_env_vars: bool = True) -> bool:
        ...
        try:
            ...
            if apply_env_vars:
                ...
                logger.info(
                    f"Applied {applied_count} environment variable configurations"
                )
            ...
            if cleanup_config.get("enabled", True):
                self.scheduler = initialize_scheduler(
                    ...
                    check_interval_minutes=cleanup_config.get(
    ...
    async def _update_existing_exports(self) -> int:
        ...
        try:
            ...
            if updated_count > 0:
                logger.info(
                    f"Updated retention information for {updated_count} existing exports"
                )
    ...
    async def get_system_status(self) -> dict:
        ...
        status = {
            ...
            "scheduler": (
                self.scheduler.get_scheduler_status()
                if self.scheduler
                else {
                    "is_running": False,
                    "enabled": False,
