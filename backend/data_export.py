...
class DataExporter:
    ...
    async def export_devices_csv(self, user_id: Optional[int] = None) -> bytes:
        ...
        writer = csv.DictWriter(
            output,
            fieldnames=[
                'device_ip', 'device_name', 'device_alias', 'device_model',
                'device_type', 'mac_address', 'location', 'group_name',
                'is_active', 'is_monitored', 'discovered_at', 'last_seen'
            ]
        )
    ...
    async def export_energy_data_csv(
        ...
        writer.writerow([
            'Device IP', 'Timestamp', 'Power (W)', 'Energy (kWh)',
            'Voltage (V)', 'Current (A)', 'Total (kWh)'
        ])
    ...
    async def _get_energy_summary(self) -> List[Dict]:
        ...
        return [dict(zip(['device_ip', 'reading_count', 'total_energy', 
                         'avg_power', 'max_power', 'min_power'], row)) 
                for row in results]
    ...
    async def _get_daily_consumption(self) -> List[Dict]:
        ...
        """
        results = await self.db_manager.execute_query(query)
        return [dict(zip(['date', 'device_ip', 'daily_energy', 
                         'avg_power', 'peak_power'], row)) 
                for row in results]
    ...
    async def _generate_recommendations(self, summary_data: Dict, devices_data: List[Dict]) -> List[str]:
        ...
        if not recommendations:
            recommendations.append("System is operating efficiently. Continue monitoring for optimization opportunities.")
