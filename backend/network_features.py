...
class NetworkDiscoveryManager:
    ...
    def _apply_vlan_config(self, vlan: VLANConfig):
        ...
        if system == "Linux":
            ...
            try:
                subprocess.run([
                    "sudo", "ip", "link", "add", "link", vlan.interface,
                    f"name", f"{vlan.interface}.{vlan.vlan_id}",
                    "type", "vlan", "id", str(vlan.vlan_id)
                ], check=True)
                
                subprocess.run([
                    "sudo", "ip", "link", "set", "dev",
                    f"{vlan.interface}.{vlan.vlan_id}", "up"
                ], check=True)
            ...
            try:
                subprocess.run([
                    "sudo", "ifconfig", f"vlan{vlan.vlan_id}",
                    "create", "vlan", str(vlan.vlan_id),
                    "vlandev", vlan.interface
                ], check=True)
            except subprocess.CalledProcessError:
                pass
    
    async def discover_devices(self, protocols: List[NetworkProtocol] = None,
                              interfaces: List[str] = None,
                              subnets: List[str] = None) -> List[DiscoveryResult]:
    ...
    def _record_discovery_start(self, discovery_id: str, interfaces: List[str],
    ...
    def get_discovered_devices(self, interface: Optional[str] = None,
                              vlan_id: Optional[int] = None) -> List[Dict]:
