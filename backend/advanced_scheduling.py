...
class AdvancedScheduler:
    ...
    def __init__(self, db_path: str = "kasa_monitor.db", 
        ...
        self.location = location or LocationInfo(
            "Default", "Default", "UTC", 0, 0
        )
        self.schedules = {}
        self.running = False
    ...
    def _create_default_templates(self, cursor):
        ...
        for template in templates:
            ...
            """, (template['name'], template['description'], 
                  template['category'], template['rules'], template['icon']))
    ...
    def update_schedule(self, schedule_id: int, updates: Dict) -> bool:
        ...
        for key, value in updates.items():
            fields.append(f"{key} = ?")
            if key in ['trigger_config', 'action_config', 'devices', 'groups', 
                      'conditions', 'metadata']:
    ...
    async def _should_trigger(self, schedule: ScheduleRule, now: datetime) -> bool:
        ...
        elif schedule.trigger_type in [TriggerType.SUNRISE, TriggerType.SUNSET,
                                       TriggerType.CIVIL_DAWN, TriggerType.CIVIL_DUSK]:
            # Solar trigger
    ...
    async def _check_solar_trigger(self, schedule: ScheduleRule, 
    ...
    def _check_last_execution(self, schedule: ScheduleRule, now: datetime) -> bool:
        ...
        """, (schedule.name,))  # Note: Using name as we need the ID
    ...
    async def _execute_device_action(self, device_ip: str, action_type: ActionType, 
                                    config: Dict[str, Any]):
    ...
    def _check_schedule_conflicts(self, schedule_id: int):
        ...
        for other_id, other_schedule in self.schedules.items():
            ...
            if (schedule.trigger_type == TriggerType.TIME and 
                other_schedule.trigger_type == TriggerType.TIME):
    ...
    def create_holiday_schedule(self, holiday_name: str, schedule_id: int,
                               country_code: str = 'US', year: Optional[int] = None) -> bool:
