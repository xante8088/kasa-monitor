...
class ExportRetentionService:
    ...
    def __init__(
        ...
        self.retention_policies = retention_policies or DEFAULT_RETENTION_POLICIES.copy()
    ...
    def _init_retention_schema(self):
        ...
        try:
            ...
            if "expires_at" not in columns:
                cursor.execute("ALTER TABLE data_exports ADD COLUMN expires_at TIMESTAMP")
            
            if "accessed_at" not in columns:
                cursor.execute("ALTER TABLE data_exports ADD COLUMN accessed_at TIMESTAMP")
            
            if "retention_period" not in columns:
                cursor.execute("ALTER TABLE data_exports ADD COLUMN retention_period INTEGER")
            
            if "status" not in columns:
                cursor.execute("ALTER TABLE data_exports ADD COLUMN status TEXT DEFAULT 'active'")
            else:
                # Update existing rows to have status if NULL
                cursor.execute("UPDATE data_exports SET status = 'active' WHERE status IS NULL")
            
            if "download_count" not in columns:
                cursor.execute("ALTER TABLE data_exports ADD COLUMN download_count INTEGER DEFAULT 0")
    ...
    async def calculate_retention_period(self, export_data: Dict) -> int:
        ...
        if file_size > LARGE_FILE_THRESHOLD:
            base_retention = min(base_retention, self.retention_policies.get("large_export", 3))
    ...
    async def update_export_retention(self, export_id: str) -> bool:
        ...
        try:
            ...
            export_data = {
                ...
                "user_role": "admin" if row[-1] else "user",  # is_admin from users table
            }
    ...
    async def cleanup_expired_exports(self, batch_size: int = 50) -> Tuple[int, int]:
        ...
        if files_deleted > 0 or records_updated > 0:
            logger.info(f"Cleanup completed: {files_deleted} files deleted, {records_updated} records updated")
        ...
        try:
            for export in exports:
                ...
                try:
                    ...
                    # Log retention audit
                    days_since_creation = (datetime.now() - datetime.fromisoformat(export["created_at"])).days
                    ...
                    await self.audit_logger.log_event_async(AuditEvent(
                        ...
                        details={
                            ...
                            "retention_period_days": export.get("retention_period", 0),
    ...
    async def check_storage_space(self) -> Dict:
        ...
        try:
            ...
            logger.info(f"Storage check - Free: {storage_info['free_gb']:.2f}GB, "
    ...
    async def emergency_cleanup(self) -> Tuple[int, int]:
        ...
        try:
            ...
            if not storage_info.get("needs_emergency_cleanup", False):
                logger.info("Emergency cleanup completed after removing expired exports")
            ...
            for export_id, file_path, file_size in large_exports:
                try:
                    if file_path and Path(file_path).exists():
                        ...
                        logger.warning(f"Emergency cleanup: deleted large export {export_id}")
                ...
                for export_id, file_path, file_size in oldest_exports:
                    try:
                        if file_path and Path(file_path).exists():
                            ...
                            logger.warning(f"Emergency cleanup: deleted old export {export_id}")
        ...
        logger.warning(f"Emergency cleanup completed: {files_deleted} files deleted, "
    ...
    async def run_daily_maintenance(self) -> Dict:
        ...
        logger.info(f"Daily maintenance completed: {len(results['tasks_completed'])} tasks, "
